...
export function Templates() {
  ...
  return (
    <section className="bg-[#030303] py-20 px-4 sm:px-6 lg:px-8 border-t border-zinc-900/60 relative">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <div className="flex justify-center gap-4">
            <Link
              href="#"
              className="inline-flex h-9 items-center justify-center rounded-md border border-zinc-800 bg-zinc-950 px-4 text-xs font-semibold text-zinc-300 hover:text-white hover:border-zinc-700 transition-colors"
            >
              View all examples
            </Link>
            <Link
              ...
              target="_blank"
              className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-zinc-800 bg-zinc-950 px-4 text-xs font-semibold text-zinc-300 hover:text-white hover:border-zinc-700 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map((tpl, idx) => (
            <div
              key={idx}
              className="group flex flex-col justify-between rounded-xl border border-zinc-900 bg-zinc-950/20 p-6 hover:border-zinc-800 hover:bg-zinc-950/60 transition-all duration-300"
            >
              <div>
                {/* Tech Logos */}
                <div className="flex flex-wrap gap-2 mb-4">
                  {tpl.tech.map((t) => (
                    <span
                      key={t}
                      className="rounded-full border border-zinc-900 bg-black px-2.5 py-0.5 text-[8.5px] font-mono font-bold text-zinc-400 group-hover:text-zinc-300 transition-colors"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <h3 className="text-sm font-bold text-white tracking-tight mb-2 group-hover:text-emerald-400 transition-colors">
                  {tpl.title}
                </h3>
              ...
              <div className="mt-6 pt-4 border-t border-zinc-900/60 flex items-center justify-between text-[10px] text-zinc-500">
                <Link
                  href={tpl.githubUrl}
